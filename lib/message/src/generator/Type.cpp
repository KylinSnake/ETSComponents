...
namespace snake
{
	namespace __protobuf__
	{
		...
		{
			name_ = name;
			auto n = snake::split(file->name(), "/.");
			proto_source_name_ = n[n.size()-2];
			namespaces_ = snake::split(file->package(), ".");
			parents_ = parent;
		}
		...
		void ClassType::output(OutputHpp& hpp)
		{
			...
			if(base_type_.empty())
			{
				...
				hpp.print_line("{ last_update = snake::DateTime::now(); }");
			...
			if(base_type_.empty())
			{
				...
				hpp.print_line("void set_last_update() { last_update = snake::DateTime::now(); }");
			}
			else
			{
		...
		{
			auto lower_name = snake::to_lower(name_);
			...
			cpp.print_line("{");
			cpp.increment_tab();
			cpp.print_line("return MessageType::" + (base_type_.empty() ? "UNKNOWN" : snake::to_upper(snake::camel_to_lower_case(name_))) + ";");
			cpp.decrement_tab();
			cpp.print_line("}");
			cpp.print_line();
		...
		bool FactoryType::parse(const Descriptor* dsp, std::vector<std::string>& parents, OutputHpp& hpp, OutputCpp& cpp)
		{
			...
			cpp.add_global_function_decl("bool handle_message_from_item(const ::snake::proto::MessageFactory::Item&, const std::function<void(const MessageBase*)>&);");
		...
		{
			cpp.print_line("bool handle_message_from_item(const ::snake::proto::MessageFactory::Item& item, const std::function<void(const MessageBase*)>& func)");
			cpp.print_line("{");
			cpp.increment_tab();
			for (auto& p : fields_)
			{
				auto name = snake::to_lower(p.first);
				auto type = p.second;
			...
			for(auto& p : fields_)
			{
				cpp.print_line("if (type == MessageType::" + snake::to_upper(snake::camel_to_lower_case(p.second)) + ")");
				cpp.print_line("{");
				cpp.increment_tab();
				...
				cpp.print_line("*(f.add_item()->mutable_" + snake::to_lower(p.first) + "()) = t;");
